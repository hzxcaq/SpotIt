...
- **[用户指南](./USER_GUIDE.md)** - 面向普通用户的完整使用指南
- **[技术文档](#技术栈)** - 开发者技术文档（本文档）
...
- **图片管理**：为物品添加照片，直观识别
- **快速搜索**：按名称、标签搜索物品，显示完整位置信息
- **自动备份**：每天自动备份数据，支持自动保存到文件夹或提示下载
- **数据恢复**：从备份历史或文件快速恢复数据
- **离线可用**：PWA 支持，无网络也能使用
...
SpotIt 提供完善的数据备份功能，确保您的数据安全：

#### 自动备份（推荐）

1. **开启自动备份**：设置页面 → 自动备份 → 开启
2. **选择备份方式**：
   - **提示下载**：每天首次访问时提示下载备份文件（兼容所有浏览器）
   - **自动保存到文件夹**：完全自动化，无需手动操作（Chrome/Edge 86+）

#### 手动备份

- **导出数据**：设置页面 → 手动备份与恢复 → 导出到文件
- **恢复数据**：设置页面 → 从文件恢复 → 选择备份文件

#### 备份存储

- **浏览器备份**：自动保留最近 7 天的备份（localStorage）
- **文件备份**：自动保存模式只保留最新 1 份（覆盖旧文件）

> 💡 **提示**：建议开启自动备份并选择云盘同步文件夹（如 OneDrive、Dropbox），实现自动云端备份。

详细使用说明请参考 **[用户指南 - 数据备份与恢复](./USER_GUIDE.md#-数据备份与恢复)**
